...
class Solution{
    private static DiameterReturn diameterOfBinaryTreeHelper(BinaryTreeNode<Integer> root){
        if (root == null){
        ...
        DiameterReturn leftOutput = diameterOfBinaryTreeHelper(root.left);
        DiameterReturn rightOutput = diameterOfBinaryTreeHelper(root.right);

        int height = 1 + Math.max(leftOutput.height, rightOutput.height);
        int diameter = 1 + leftOutput.height + rightOutput.height;
    ...
    public static int diameterOfBinaryTree(BinaryTreeNode<Integer> root){
        DiameterReturn ansLeft = diameterOfBinaryTreeHelper(root.left);
        DiameterReturn ansRight = diameterOfBinaryTreeHelper(root.right);

        int leftHeightPlusRightHeigt = ansLeft.height + ansRight.height + 1;
        int leftDiameter = ansLeft.diameter;
        int rightDiameter = ansRight.diameter;

        return Math.max(leftHeightPlusRightHeigt, Math.max(leftDiameter, rightDiameter));
    }
